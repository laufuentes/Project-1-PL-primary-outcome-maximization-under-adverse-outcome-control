...
lambda_evol<- function(results,option, lambda_opt, beta_opt){
  ...
  ggsave(paste0("images/oracular/lambda_evol_",option[1],"_",option[2],".pdf"),lambda_evol)
  ...
  ggsave(paste0("images/oracular/beta_evol_",option[1],"_",option[2],".pdf"),beta_evol)
}
...
gamma_plot_funct <- function(results_optimal_x, lambda_value, beta_value, option, df) {
...
geom_points_fct <- function(results, idx_opt, df, option){
  ...
  plots <- lapply(lambda_discr, function(x) gamma_plot_funct(results$optimal_x[[x]], results$lambda[[x]], results$beta[[x]],option, df))
  ...
  print(final_plot)
  ggsave(paste0("images/oracular/geom_points_lambda_",option[1],"_",option[2],".pdf"),final_plot)
  
  plot_none<-gamma_plot_funct(results$optimal_x[[1]], results$lambda[[1]], results$beta[[1]], option, df)
  plot_min <- gamma_plot_funct(results$optimal_x[[idx_opt-1]], results$lambda[[idx_opt-1]], results$beta[[idx_opt-1]], option, df)
  plot_max <- gamma_plot_funct(results$optimal_x[[idx_opt]], results$lambda[[idx_opt]], results$beta[[idx_opt]], option, df)

  opt_plots <- plot_grid(plot_none, plot_min,plot_max, ncol=3)
  ggsave(paste0("images/oracular/geom_points_lambda_opt_",option[1],"_",option[2],".pdf"),opt_plots)
...
gamma_lambda_plot <- function(results,option, df){
  ...
  ggsave(paste0("images/oracular/lambda_plot_",option[1],"_",option[2],".pdf"),lambda_plot)
...
animation_plot<- function(results, lambda_values, option, df){
  ...
  if(option[1]=="IVF"){
    ...
    df_policy <- as.data.frame(sapply(results$optimal_x, function(x){sigma_beta(x, beta)}))
  ...
  anim_save(paste0("images/oracular/policy_animation_",option[1],"_",option[2],".gif"))
