...
synthetic_setting_plot <- function(df_complete, option, type_simu) {
  ...
  if (option[1] == "IVF") {
    plot_Y_sign <- ggplot(
      df_complete,
      aes(x = X.height, y = X.bmi, color = sign_CATE)
    ) +
    ...
    p_plot <- ggplot(
      df_complete,
      aes(x = X.height, y = X.bmi, color = (delta_Z))
    ) +
      geom_point(alpha = 0.5) +
    ...
    if (option[2] == "2") {
      p_plot <- p_plot +
        geom_rect(
          aes(xmin = min(X.height), xmax = 150, ymin = min(X.bmi), ymax = 20),
          color = "black",
          fill = NA,
          linewidth = 1,
          inherit.aes = FALSE
        ...
        geom_rect(
          aes(xmin = min(X.height), xmax = 155, ymin = min(X.bmi), ymax = 25),
          color = "black",
          fill = NA,
          linewidth = 1,
          inherit.aes = FALSE
    ...
    plot_Y_sign <- ggplot(
      df_complete,
      aes(x = X.1, y = X.2, color = sign_CATE)
    ) +
      geom_point(alpha = 0.5)
    p0_plot <- ggplot(df_complete, aes(x = X.1, y = X.2, color = (p0))) +
      ...
      scale_color_gradient(
        low = "yellow",
        high = "red",
        limits = c(min(df_complete$p0), max(df_complete$p1))
      )
  ...
  ggsave(
    paste0("images/synthetic_setting_", option[1], "_", option[2], ".pdf"),
    combined_plot
  )
}

lambda_evol <- function(results, option, lambda_opt, beta_opt) {
  ...
  ggsave(
    paste0("images/oracular/lambda_evol_", option[1], "_", option[2], ".pdf"),
    lambda_evol
  )
  ...
  ggsave(
    paste0("images/oracular/beta_evol_", option[1], "_", option[2], ".pdf"),
    beta_evol
  )
}

policy_value_plot <- function(results, option, lambda_opt, beta_opt) {
  results_beta <- results[which(results$lambda == lambda_opt), ]
  results_lambda <- results[which(results$beta == beta_opt), ]
  # lambda plot
  lambda_evol <- ggplot(results_lambda %>% as.data.frame(), aes(x = lambda)) +
    ...
    scale_color_manual(
      name = "Functions",
      values = c("Policy value" = "black", "L" = "gray")
    ) +
    theme_minimal()
  ...
  beta_evol <- ggplot(results_beta, aes(x = beta)) +
    ...
    scale_color_manual(
      name = "Functions",
      values = c("Risk" = "blue", "Constraint" = "red", "L" = "gray")
    ) +
    theme_minimal()
...
gamma_plot_funct <- function(
  ...
  policy <- sigma_beta(results_optimal_x)
  ...
  if (option[1] == "IVF") {
    p <- ggplot(
      ...
      labs(
        title = bquote(lambda == .(lambda_value) ~ "," ~ beta == .(beta_value))
      ) +
      theme_minimal() +
...
geom_points_fct <- function(results, idx_opt, df, option) {
  lambda_discr <- which(results$beta == results$beta[idx_opt])[as.integer(seq(
    1,
    length(which(results$beta == results$beta[idx_opt])),
    length.out = 10
  ))]
  ...
  ggsave(
    paste0(
      "images/oracular/geom_points_lambda_",
      option[1],
      "_",
      option[2],
      ".pdf"
  ...
  plot_min <- gamma_plot_funct(
    results$optimal_x[[idx_opt - 1]],
    results$lambda[[idx_opt - 1]],
    results$beta[[idx_opt - 1]],
    option,
    df
  )
  plot_max <- gamma_plot_funct(
    results$optimal_x[[idx_opt]],
  ...
  ggsave(
    paste0(
      "images/oracular/geom_points_lambda_opt_",
      option[1],
      "_",
      option[2],
      ".pdf"
...
gamma_lambda_plot <- function(results, option, df) {
  ...
  ggsave(
    paste0("images/oracular/lambda_plot_", option[1], "_", option[2], ".pdf"),
    lambda_plot
  )
  ...
  ggsave(
    paste0("images/oracular/beta_plot_", option[1], "_", option[2], ".pdf"),
    beta_plot
  )
  ...
  ggsave(
    paste0(
      "images/oracular/geom_beta_plot_",
      option[1],
      "_",
      option[2],
      ".pdf"
...
animation_plot <- function(results, lambda_values, option, df) {
  # For lambda
  if (option[1] == "IVF") {
    df_covariates <- df %>%
    ...
    df_policy <- as.data.frame(sapply(results$optimal_x, function(x) {
      sigma_beta(x, beta)
    }))
    ...
    p <- ggplot(df_final, aes(x = X.height, y = X.bmi, color = policy)) +
      ...
      labs(
        title = "Treatment Probability for λ = {closest_state}",
        x = "Height",
        y = "BMI",
  ...
  anim_save(paste0(
    "images/oracular/policy_animation_",
    option[1],
    "_",
    option[2],
    ".gif"
