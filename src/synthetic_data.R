...
data_gen <- function(n, option) {
  if (option[1] == "IVF") {
    ...
    Y_0 <- as.integer(pmax(
      u - (as.matrix(X) %*% theta_Y0) + rnorm(n, mean = 0, sd = 1),
      0
    ))
    Y_1 <- as.integer(pmax(
      Y_0 + beta * (1 - as.matrix(X) %*% theta_Y1) + rnorm(n, mean = 0, sd = 1),
      0
    ))
    ...
    p0 <- ifelse(
      ...
      0.055
    ) # Generate random values for false condition
  ...
  df_obs <- data.frame(
    ...
    Z = ifelse(Treatment == 1, p1, p0)
  )
  return(list(df_complete, df_obs))
}

delta_Y <- function(X, option) {
  if (option[1] == "IVF") {
    u <- 10
    beta <- 3
    theta_Y0 <- c(-0.01, 0.2, -0.05, 1.5 / 4, 2.5 / 4, 1.5 / 4, 1.5 / 4)
    ...
    Y1_X <- as.integer(
      pmax(
        Y0_X + beta * (1 - as.matrix(X) %*% theta_Y1),
        0
      )
    )
  } else {
